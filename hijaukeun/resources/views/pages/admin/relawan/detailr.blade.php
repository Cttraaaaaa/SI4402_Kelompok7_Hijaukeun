    ...
    <div class="w-full mb-8 overflow-hidden rounded-lg shadow-xs">
      <div class="w-full overflow-x-auto">
        ...
        <div
          ...
          <h2 class="mt-4">NIK : {{ $item->nik }}</h2>
          <h2 class="mt-4">No Telepon : {{ $item->nohp }}</h2>
          <h2 class="mt-4">Tanggal : {{ $item->created_at->format('l, d F Y - H:i:s') }}</h2>
          <h2 class="mt-4">Status : 
            @if($item->status =='Belum di Acc')
            <span
        ...
        <div class="px-4 py-3 mb-8 flex text-gray-800 bg-white rounded-lg shadow-md dark:bg-gray-800">
          <div class="text-center flex-1 dark:text-gray-400">
            <h1 class="mb-8 font-semibold">Alamat</h1>
